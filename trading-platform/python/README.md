...
API reference: [PlaceGTTOrder](../../api-documentation/v1/orders-api/placegttorder.md),
...
The public webhook at `/strategy/webhook/<token>` is what TradingView and other alert
senders post to. It is not under `/api/v1` and takes no API key: the `oaws_` token in
the URL is the whole credential. The SDK's `Strategy` class speaks its protocol:
...
Every documented outcome is returned rather than raised, with its `result` label.
See [Strategy RMS From Python](strategy-management.md) for the full webhook guide,
and the [Strategy RMS API](../../api-documentation/v1/strategy-rms-api/README.md)
for every field of the nine methods above.
