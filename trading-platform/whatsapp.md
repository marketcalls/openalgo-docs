...
Slash-command queries (`/orderbook`, `/positions`, and the rest) execute against the OpenAlgo SDK using **your own** OpenAlgo API key, looked up server-side by the owner username captured at pair time. If you haven't generated one yet:
...
* `/mode`: Show whether the OpenAlgo instance is in `live` or `analyze` (sandbox) mode. Read-only; the WhatsApp bot cannot switch modes
...
3. The API key on the order must resolve to the OpenAlgo username recorded as `owner_username` at pair time. Orders placed by a different OpenAlgo user do not alert the paired device.
4. Orders must be placed through the OpenAlgo API (REST `/api/v1/*`, the Python SDK, or any tool that ultimately hits the API).
...
The `whatsapp()` method ships in the `openalgo` package. OpenAlgo currently pins `openalgo==2.0.3`.
...
| `username` | string | OpenAlgo username, resolved via the linked-users table                                  |
...
Slash-commands are gated by WhatsApp's own multi-device cryptography. When the operator types `/orderbook` from their primary phone, WhatsApp marks the message as `is_from_me=True` when mirroring it to the linked OpenAlgo device. Random contacts who message the operator's number arrive with `is_from_me=False`. The bot's handler unconditionally drops the latter. There is no allowlist to maintain and no `/link` flow to manage.
...
Linked recipient phone numbers and their OpenAlgo username and api\_key mapping. This table stays empty in the standard single-user deployment: there is no `/link` command on the WhatsApp bot, so rows only appear if a deployment populates them itself.
...
* **Low risk (typical OpenAlgo usage)**: A handful of self-send order alerts per day, occasional `/status` replies, sending charts or reports to a small circle of subscribers. Indistinguishable from a person using WhatsApp normally, and well under Meta's automated thresholds.
...
1. Check the server logs (`log/openalgo_YYYY-MM-DD.log` plus `log/errors.jsonl`)
2. Open `/whatsapp` and inspect the status badge and pair-state JSON via `GET /whatsapp/pair/status`
3. Verify wars is installed: `uv run python -c "import wars; print(wars.__version__)"`. OpenAlgo pins `wars==0.1.4`
